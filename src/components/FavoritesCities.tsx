...
interface FavoritesCityTabletProps {
    id: string;
    name: string;
    lat: number;
    lon: number;
  ...
  return (
    ...
    <ScrollArea className=" w-full pb-4">
        <div className=" flex gap-4">
            {favorites.map((city)=>{
                return (
                    <FavoritesCityTablet key={city.id} {...city} onRemove={()=>removeFavorites.mutate(city.id)}/>
                )
            })}
        </div>
    </ScrollArea>
...
function FavoritesCityTablet({id, name, lat,lon , onRemove}:FavoritesCityTabletProps) {
